...
use audio_ninja::{
    loudness::{
        DynamicRangeControl, HeadroomManager, LoudnessMeter, LoudnessNormalizer,
        LoudnessTarget,
    },
...
fn main() -> anyhow::Result<()> {
    ...
    println!("   Peak before limiting: {:.3} ({:.2} dBFS)", peak_before, linear_to_db(peak_before));
    println!("   Peak after limiting: {:.3} ({:.2} dBFS)\n", peak_after, linear_to_db(peak_after));
    ...
    println!("     - Headroom: -3dB protection\n");
    ...
    println!("  Peak:           {:.3} ({:.2} dBFS)", find_peak(&test_audio), linear_to_db(find_peak(&test_audio)));
    println!();
    println!("After Normalization:");
    println!("  Loudness:       {:.2} LUFS (target: -14)", normalized_loudness);
    println!("  Gain applied:   {:.2} dB", gain_db);
    println!();
    println!("After DRC (4:1 compression):");
    println!("  Loudness:       {:.2} LUFS", drc_integrated);
    println!("  Range reduced:  {:.2} LU ({:.0}% reduction)", 
        original_range - drc_range,
        ((original_range - drc_range) / original_range.max(0.1) * 100.0));
    println!();
    println!("After Headroom Protection:");
    println!("  Peak:           {:.3} ({:.2} dBFS)", peak_after, linear_to_db(peak_after));
    ...
    println!("✓ EBU R128: Integrated loudness = {:.2} LUFS", normalized_loudness);
    ...
    for _ch in 0..num_channels {
        ...
        for i in 0..num_samples {
            ...
            let sample = if t < 0.2 {
                ...
                0.8 * peak_envelope * (2.0 * std::f32::consts::PI * 880.0 * (i as f32 / sample_rate as f32)).sin()
            } else {
